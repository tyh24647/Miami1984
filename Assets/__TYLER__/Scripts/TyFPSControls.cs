...
public class TyFPSControls : vp_CharacterController {
    ...
    protected override void Start() {
        ...
        if (MainTerrain == null) {
            MainTerrain = FindObjectOfType<Terrain>();
        }
    }
    ...
    protected virtual void UpdateJumpForceFree() {
        ...
        if (Player.Jump.Active)
            ...
            if (Grounded && Mathf.Approximately(CharacterController.height, m_NormalHeight)) {
        ...
        // start sliding if ground is steep enough in angles
            ...
            if (GroundAngle <= Player.SlopeLimit.Get()) {
            ...
            } else    // if steeper than slopelimit, slide with accumulating slide speed
              {
                ...
                if (Mathf.Approximately(m_OnSteepGroundSince, 0.0f)) {
    ...
    protected virtual void SmoothMove() {
        ...
        //if (Time.timeScale == 0.0f)
        if (Time.timeScale.CompareTo(0.0f) == 0) {
            return;
        }
    ...
    protected virtual void DeflectUpForce() {
        ...
        Player.HeadImpact.Send(((m_NewDir.x < 0.0f) || (Mathf.Approximately(m_NewDir.x, 0.0f) && (Random.value < 0.5f))) ? -m_ForceImpact : m_ForceImpact);
