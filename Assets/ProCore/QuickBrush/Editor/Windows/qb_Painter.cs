...
public class qb_Painter : EditorWindow {
    ...
    private void LoadToolState() {  //Debug.Log("Reloading Tool State!");
	...
	*/
    ...
    void PrefabTile(int i) {
        ...
        if (liveTemplate.selectedPrefabIndex == i) {    //prefabSelectedTexture = selectPrefabCheckTexture_hover;
        ...
        } else {    //picButton_PrefabCheck. = selectPrefabCheckTexture_normal;
        ...
        for (int ii = 0; ii < objects.Length; ii++) {
            ...
            if (Vector3.Distance(objects[ii].transform.position, newPoint.position) < curTemplate.brushRadius) {
                ...
                if (eraseGrouped == true) {
                    if (groupIsNothing == false) {  //Regular, group based
    ...
    static void UpdateCompoundPaintToSelection() {  //Debug.Log("Updating Compound Paint To Selection");
        int count = window.brushTemplates.Length;
    ...
    private void SwitchToTab(int tabIndex) {    //Debug.Log(tabIndex);
        if (tabIndex == -1)
    ...
    private void SaveTempSlots() {  //Debug.Log("Saving Temp Slots");
