...
namespace Zios {
    ...
    public static class ColorExtension {
        ...
        public static Color Blend(this Color current, Color target, ColorBlend mode, float amount = 1) {
            ...
            } else if (mode.Matches("Difference")) { value = (current - target).Abs(); } else if (mode.Matches("Exclusion")) { value = (current + target - 2.0f * current * target); } else if (mode.Matches("Subtract")) { value = target - current; } else if (mode.Matches("Divide")) { value = target.Divide(current); } else if (mode.Matches("Hue")) { } else if (mode.Matches("Saturation")) { } else if (mode.Matches("Color")) { } else if (mode.Matches("Luminosity")) { }
            if (!formula.IsNull()) {
