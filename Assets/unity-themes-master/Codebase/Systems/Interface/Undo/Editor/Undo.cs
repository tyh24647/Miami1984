        ...
        public static void Handle(string data) {
            ...
            foreach (var change in items) {
                ...
                if (change.Contains("###")) {
                    ...
                    if (type == typeof(string)) { scope.SetVariable(field, value); } else if (type == typeof(int)) { scope.SetVariable(field, value.ToInt()); } else if (type == typeof(float)) { scope.SetVariable(field, value.ToFloat()); } else if (type == typeof(bool)) { scope.SetVariable(field, value.ToBool()); } else if (type.IsEnum) { scope.SetVariable(field, Enum.Parse(type, value)); }
                    Undo.snapshot.Remove(scope);
                } else {
                    var key = change.Split("|||")[0];
                    var type = change.Split("|||")[1].Split("---")[0];
                    var value = change.Split("|||")[1].Split("---")[1];
                    if (type.Contains("Bool")) { Utility.SetPref<bool>(key, value.ToBool()); } else if (type.Contains("Int")) { Utility.SetPref<int>(key, value.ToInt()); } else if (type.Contains("String")) { Utility.SetPref<string>(key, value); } else if (type.Contains("Float")) { Utility.SetPref<float>(key, value.ToFloat()); }
                }
            }
            Undo.snapshotPrefs.Clear();
        }
