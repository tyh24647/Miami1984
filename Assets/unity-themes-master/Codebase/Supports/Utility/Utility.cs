...
namespace Zios {
    ...
    public static partial class Utility {
        ...
        public static void Destroy(UnityObject target, bool destroyAssets = false) {
            ...
            if (!Application.isPlaying) { UnityObject.DestroyImmediate(target, destroyAssets); } else { UnityObject.Destroy(target); }
        }
        ...
        public static void Log(object key, string text, UnityObject target, LogType type, int limit) {
            if (Utility.messages.AddNew(key) < limit || limit == -1) {
                ...
                if (type == LogType.Debug) { Debug.Log(text, target); } else if (type == LogType.Warning) { Debug.LogWarning(text, target); } else if (type == LogType.Error) { Debug.LogError(text, target); }
            }
        }
