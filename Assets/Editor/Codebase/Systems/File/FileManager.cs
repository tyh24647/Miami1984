...
namespace Zios {
    ...
    public static class FileManager {
        ...
        public static void Load() {
            ...
            if (FileManager.Exists(cachePath)) {
                ...
                for (int index = 0; index < lines.Length; ++index) {
                    ...
                    if (line.StartsWith("(")) { extension = line.Parse("(", ")"); } else if (line.StartsWith("=") || line.StartsWith("+")) {
        ...
        public static void SaveData(FileData data, StreamWriter output, ref string lastPath) {
            ...
            if (directory == lastPath) { } else if (directory.Contains(lastPath)) {
        ...
        public static FileData[] FindAll(string name, bool showWarnings = true, bool returnFirstMatch = false) {
            ...
            if (type.IsEmpty() || type == "*") { types = allTypes.ToList(); } else if (type.StartsWith("*")) { types.AddRange(allTypes.Where(x => x.EndsWith(type.Remove("*"), true))); } else if (type.EndsWith("*")) { types.AddRange(allTypes.Where(x => x.StartsWith(type.Remove("*"), true))); } else if (FileManager.filesByType.ContainsKey(type)) { types.Add(type); }
