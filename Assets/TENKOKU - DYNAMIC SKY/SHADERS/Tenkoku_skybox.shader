...
half atFac = saturate(lerp(_Tenkoku_HorizonHeight,1.0,dot(half3(0,1,0), (-eyeRay))));// * (1.0-saturate(dot(-eyeRay,Tenkoku_Vec_SunFwd))*0.95);
...
//float kScaleOverScaleDepth = (1.0 / (OUTER_RADIUS - 1.0)) / clamp(lerp(0.1,0.5, saturate(dot(half3(0,1,0), normalize(-eyeRay.xyz))) ),0.1,0.25);
...
col.rgb = lerp(col.rgb,half3(col.r,col.r,col.r)*_Tenkoku_AmbientGI,saturate(_Tenkoku_overcastColor.a*2.0));
