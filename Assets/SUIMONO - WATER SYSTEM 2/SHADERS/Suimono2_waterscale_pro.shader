...
Properties {
	...
	_suimono_uv3x ("uvx2", Float) = 1.0
	_suimono_uv3y ("uvy2", Float) = 1.0
	
	_suimono_uv4x ("uvx2", Float) = 1.0
	_suimono_uv4y ("uvy2", Float) = 1.0
	...
	suimonoDiffHeight ("Difference Height", Float) = 1.0
...
inline fixed4 LightingSuimonoDepth (SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
{
	...
	half htFac = suimonoDiffHeight/40.0;
	cP.a = saturate(lerp(3.5,-2.5+htFac,dot(half3(0,0,1),viewDir)));
	cP.a -= (1.0-s.Alpha);
	cP.a = saturate(cP.a);
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	float mask0 = saturate((IN.screenPos.w - 15.0)*0.1);
...
inline fixed4 LightingSuimonoDepth (SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
{
	...
	//scale alphas
	//half htFac = suimonoDiffHeight/40.0;
	//a *= saturate(lerp(2.0,-3.5+htFac,dot(half3(0,1,0),viewDir)));
	//a -= (1.0-s.Alpha);
	//a = saturate(a);
...
#pragma target 3.0
#include "SuimonoFunctions.cginc"
#pragma surface surf SuimonoDepth addshadow nolightmap noambient alpha
#pragma glsl
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//calculate distance mask
	float mask0 = saturate((IN.screenPos.w - 15.0)*0.1);
	mask = saturate((uv0.w - lerp(66.0,20.0,(_ReflDist/50.0)))*_ReflBlend);
	mask1 = saturate((uv0.w - lerp(166.0,20.0,(5.0/25.0)))*0.002);
	mask2 = saturate((uv0.w - lerp(6.0,20.0,(5.0/25.0)))*0.01);
	mask3 = saturate((uv0.w - lerp(-144.0,60.0,(10.0/25.0)))*0.01);
	maskD = saturate((uv0.w - lerp(86.0,20.0,(5.0/25.0)))*0.01);
	maskcastshadow = saturate((uv0.w - lerp(0.0,60.0,(_castshadowFade/100.0)))*0.01);
