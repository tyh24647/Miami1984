...
Tags {"RenderType"="Transparent" "Queue"= "Geometry-1"} //overlay+1
Cull Back
ZWrite On
...
inline fixed4 LightingSuimonoDepth (SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
{
	...
	//orig color
	half3 origColor = saturate(refractColor.rgb*1.8);
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	half htAdd = (saturate(IN.worldPos.y-(suimonoHeight+((_suimono_DeepWaveHeight+_suimono_DetailHeight)*0.15)))*0.3);
	depthPos = saturate(depthPos+htAdd);
...
half4 odepth3 = tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(uvx)) * 1.0;
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//pixel refraction low level test
	//half yFac = lerp(0.0,(1.0*clamp(o.Normal.y,0.0,0.05))*(IN.screenPos.w*0.02)*0.4,IN.screenPos.w); 
	//uv1.y -= (0.1*_ReflectStrength) * lerp(yFac*4.0,yFac*0.5,1.0/IN.screenPos.w);
	
	//stretch movement test
	//half sFac = lerp(1.0-(norm1.y*0.3),0.0,saturate(1.0/IN.screenPos.w));
	//uv1.y = (uv1.y * sFac) - (lerp(lerp(lerp(0.0,-50.0,(1.0/IN.screenPos.w)),0.0,-norm1.y),55.0*norm1.z,saturate(1.0/IN.screenPos.w)));
	
	//yFac -= lerp(norm2.y*0.8,0.0,1.0/IN.screenPos.w);
	...
	//ocean edge blend
	//half oceanEdgeTerm = tex2D(_OceanBlendTex, IN.screenPos.xy).r;
	//o.Alpha *= oceanEdgeTerm;
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//o.Albedo = getflowmap.rgb;//half3(flowmap.x,flowmap.y,0.0);
